package com.juanpoveda.cocktails.repository
...
class FakeCocktailsRepository: CocktailsRepository {
    ...
    private val cocktail2 = Cocktail (
        ...
        ingredients = hashMapOf(
            "Whiskey" to "2oz",
            "Angostura bitters" to "a dash",
            "Orange peel" to "a twist",
        )
    )

    private val cocktailFakeList = listOf(cocktail1, cocktail2)

    private val ingredientsFakeList = listOf(
        Ingredient(
            id = "1",
            name = "Vodka",
    ...
    override fun getCocktailList(forceRefresh: Boolean): Flow<Result<List<Cocktail>>> {
        return flow { emit(Result.Success(data = cocktailFakeList)) }
    }

    override fun getIngredient(ingredientName: String): Flow<Result<Ingredient>> {
        return flow { emit(Result.Success(data = ingredientsFakeList.first { it.name == ingredientName })) }
    }
